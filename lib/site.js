...
* begin_port - Number. Firt port which listen. Other side listen on
  begin_port + i.
*/
async function listen(docs, credentials, begin_port, address) {
    let sites = [];

    for (let i = 0; i < docs.length; ++i) {
        let site = await deploy(docs[i], credentials, begin_port + i, address);
...
async function deploy(doc, credentials, port, address) {
